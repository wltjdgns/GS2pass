...
renderCUDA(
    ...
    const float* __restrict__ normals,         // NEW: 각 Gaussian의 법선 (N×3)
    const float* __restrict__ final_Ts,
    const uint32_t* __restrict__ n_contrib,
    const float* __restrict__ dL_dpixels,
    const float* __restrict__ dL_invdepths,
    const float* __restrict__ dL_depths,       // NEW: depth loss gradient (H×W)
    const float* __restrict__ dL_normals, 
    // === 추가 ===
    const float* __restrict__ dL_basecolors,     // (H×W×3)
    const float* __restrict__ dL_roughness,      // (H×W)
    ...
    float* __restrict__ dL_d_depths_out,       // NEW: depth parameter gradient (N×1)
    float* __restrict__ dL_d_normals_out,
    // === 추가 ===
    float* __restrict__ dL_d_basecolors_out,     // (N×3)
    float* __restrict__ dL_d_roughness_out       // (N×1)
)
{
    ...
    // NEW: Normal shared memory (3 channels)
    __shared__ float collected_normals[3 * BLOCK_SIZE];
	
	// === 추가: Basecolor/Roughness shared memory ===
	__shared__ float collected_basecolors[3 * BLOCK_SIZE];
	...
	float accum_normal_rec[3] = { 0.0f, 0.0f, 0.0f };

	// === 추가: Basecolor/Roughness ===
	float dL_basecolor[3];
	float dL_rough;
	float accum_basecolor_rec[3] = { 0.0f, 0.0f, 0.0f };
	float accum_roughness_rec = 0.0f;
	...
	if (inside)
	{
		...
		{
			for (int c = 0; c < 3; c++)
				dL_normal[c] = dL_normals[c * H * W + pix_id];
		}
		
		// === 추가: Load basecolor/roughness gradients ===
		if (dL_basecolors)
		{
			for (int c = 0; c < 3; c++)
				dL_basecolor[c] = dL_basecolors[c * H * W + pix_id];
		}
		if (dL_roughness)
		{
			dL_rough = dL_roughness[pix_id];
		}
	}
    ...
    // NEW: Track last depth/normal for blending
    float last_depth = 0.0f;
    float last_normal[3] = { 0.0f, 0.0f, 0.0f };
	// === 추가 ===
	float last_basecolor[3] = { 0.0f, 0.0f, 0.0f };
	float last_roughness = 0.0f;
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        if (range.x + progress < range.y)
        {
            ...
            // NEW: Load normal data
            if (dL_normals)
            {
                for (int c = 0; c < 3; c++)
                    collected_normals[c * BLOCK_SIZE + block.thread_rank()] 
                        = normals[coll_id * 3 + c];
            }

			// === 추가: Load basecolor/roughness ===
			if (dL_basecolors)
			{
				for (int c = 0; c < 3; c++)
					collected_basecolors[c * BLOCK_SIZE + block.thread_rank()] 
						= basecolors[coll_id * 3 + c];
			}
			if (dL_roughness)
			{
        ...
        for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
        {
            ...
            if (dL_depths)
            {
                const float d_val = collected_depths[j];
                accum_depth_rec = last_alpha * last_depth + (1.f - last_alpha) * accum_depth_rec;
                last_depth = d_val;
                dL_dalpha += (d_val - accum_depth_rec) * dL_depth;
                atomicAdd(&(dL_d_depths_out[global_id]), dchannel_dcolor * dL_depth);
            }
            
            // ===== NEW: Normal gradient (3 channels) =====
			if (dL_normals)
			{
				for (int c = 0; c < 3; c++)
				{
					const float n_val = collected_normals[c * BLOCK_SIZE + j];
					accum_normal_rec[c] = last_alpha * last_normal[c] + (1.f - last_alpha) * accum_normal_rec[c];
					last_normal[c] = n_val;
					dL_dalpha += (n_val - accum_normal_rec[c]) * dL_normal[c];
					atomicAdd(&(dL_d_normals_out[global_id * 3 + c]), dchannel_dcolor * dL_normal[c]);
				}
			}

			// ===== 추가: Basecolor gradient (3 channels) =====
			if (dL_basecolors)
			{
				for (int c = 0; c < 3; c++)
				{
					const float b_val = collected_basecolors[c * BLOCK_SIZE + j];
					accum_basecolor_rec[c] = last_alpha * last_basecolor[c] + (1.f - last_alpha) * accum_basecolor_rec[c];
					last_basecolor[c] = b_val;
					dL_dalpha += (b_val - accum_basecolor_rec[c]) * dL_basecolor[c];
					atomicAdd(&(dL_d_basecolors_out[global_id * 3 + c]), dchannel_dcolor * dL_basecolor[c]);
				}
			}

			// ===== 추가: Roughness gradient (1 channel) =====
			if (dL_roughness)
			{
				const float r_val = collected_roughness[j];
				accum_roughness_rec = last_alpha * last_roughness + (1.f - last_alpha) * accum_roughness_rec;
				last_roughness = r_val;
				dL_dalpha += (r_val - accum_roughness_rec) * dL_rough;
				atomicAdd(&(dL_d_roughness_out[global_id]), dchannel_dcolor * dL_rough);
			}
...
void BACKWARD::render(
    ...
    float* dL_dcolors,
    float* dL_dinvdepths,
    float* dL_ddepths_out,
    float* dL_dnormals_out,
    float* dL_dbasecolors_out,
    float* dL_droughness_out)
{
