...
class GaussianModel:
    ...
    def __init__(self, sh_degree, optimizer_type="default"):
        ...
        # ✅ 추가: BRDF 파라미터
        self._basecolor = torch.empty(0)  # (N, 3) - Albedo
        self._roughness = torch.empty(0)  # (N, 1) - 거칠기 [0,1]
        self._metallic = torch.empty(0)   # (N, 1) - 금속성 [0,1] (선택)
    ...
    def get_basecolor(self):
        """Basecolor (albedo) - (N, 3) RGB"""
        return torch.sigmoid(self._basecolor)  # [0, 1] 범위

    @property
    def get_roughness(self):
        """Roughness - (N, 1)"""
        return torch.sigmoid(self._roughness)  # [0, 1]

    @property  
    def get_metallic(self):
        """Metallic - (N, 1)"""
        return torch.sigmoid(self._metallic)  # [0, 1]
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float):
        ...
        basecolor_init = torch.logit(fused_color_rgb.clamp(0.01, 0.99))  # inverse sigmoid
        self._basecolor = nn.Parameter(basecolor_init.requires_grad_(True))
        
        # Roughness: 중간값 0.5로 초기화
        roughness_init = torch.zeros((num_points, 1), device="cuda")  # sigmoid(0) = 0.5
        self._roughness = nn.Parameter(roughness_init.requires_grad_(True))
        
        # Metallic: 낮은 값 0.1로 초기화
        metallic_init = torch.ones((num_points, 1), device="cuda") * (-2.2)  # sigmoid(-2.2) ≈ 0.1
    ...
    def construct_list_of_attributes(self):
        ...
        # ✅ 추가: BRDF 속성
        for i in range(3):  # basecolor RGB
    ...
    def save_ply(self, path):
        ...
        # ✅ BRDF 파라미터 저장
        basecolor = self.get_basecolor.detach().cpu().numpy()
    ...
    def load_ply(self, path, use_train_test_exp = False):
        ...
        # ===== ✅ 기존: BRDF 로딩 =====
        try:
            basecolor_names = [p.name for p in plydata.elements[0].properties 
                            if p.name.startswith("basecolor")]
            basecolor_names = sorted(basecolor_names, key=lambda x: int(x.split('_')[-1]))
            basecolor = np.zeros((xyz.shape[0], len(basecolor_names)))
            for idx, attr_name in enumerate(basecolor_names):
                basecolor[:, idx] = np.asarray(plydata.elements[0][attr_name])
            
            roughness = np.asarray(plydata.elements[0]["roughness"])[..., np.newaxis]
            metallic = np.asarray(plydata.elements[0]["metallic"])[..., np.newaxis]
            
            # inverse sigmoid (원본값 그대로 저장)
            ...
            print("⚠️ BRDF parameters not found in .ply, initializing with defaults")
            # 기본값으로 초기화
            num_points = xyz.shape[0]
            self._basecolor = nn.Parameter(torch.zeros((num_points, 3), device="cuda").requires_grad_(True))
            self._roughness = nn.Parameter(torch.zeros((num_points, 1), device="cuda").requires_grad_(True))
            self._metallic = nn.Parameter(torch.ones((num_points, 1), device="cuda") * (-2.2).requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        self._metallic = self._metallic[valid_points_mask]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii):
        ...
        new_roughness = torch.zeros((N_new, 1), device="cuda")  # sigmoid(0) = 0.5
        new_metallic = torch.ones((N_new, 1), device="cuda") * (-2.2)  # sigmoid(-2.2) ≈ 0.1
        
        self._basecolor = torch.cat([self._basecolor, new_basecolor], dim=0)
